...
def build_proxies_dict(
    ...
    scheme : str
        Proxy scheme for ``requests``. Supports ``http``, ``socks4``,
        ``socks5`` and ``socks5h`` when the corresponding requests extra is
        installed. ``socks5h`` keeps DNS resolution on the proxy side.
    ...
    if scheme not in {"http", "socks4", "socks5", "socks5h"}:
        raise ValueError("unsupported proxy scheme: {}".format(scheme))
    if user and pwd:
        url = "{}://{}:{}@{}:{}".format(
            scheme,
            urllib.parse.quote(str(user), safe=""),
            urllib.parse.quote(str(pwd), safe=""),
            host,
            port,
        )
    else:
...
def apply_smart_fingerprint(
    ...
    *,
    ...
    proxy_host / proxy_port / proxy_user / proxy_pwd / proxy_scheme
        Proxy info; omit host/port for direct connection.
    ...
    # 1) proxies dict
    proxy_scheme = (proxy_scheme or "http").lower()
    if proxy_scheme == "https":
        proxy_scheme = "http"
    if proxy_scheme == "socks":
        proxy_scheme = "socks5"
    proxies = build_proxies_dict(proxy_host, proxy_port, proxy_user, proxy_pwd, proxy_scheme)
    ...
    if set_proxy_on_opts and proxy_host and proxy_port:
        try:
            opts.set_proxy("{}://{}:{}".format(proxy_scheme, proxy_host, proxy_port))
        except Exception as e:  # noqa: BLE001
